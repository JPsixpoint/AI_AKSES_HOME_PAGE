...
// Use the specific Neon database URL provided by the user
...
export async function verifyDatabaseConnection() {
  try {
    ...
    try {
      ...
      console.log(`Successfully connected to akses_deals table with ${testQuery.rows[0].count} records`);
      usingSqliteFallback = false;
      return true;
    } catch (error) {
      const tableError = error as Error;
      console.error('Error accessing akses_deals table:', tableError.message);
